    ...
    <div className="bg-yellow-100 p-4 rounded shadow relative">
      <div className="flex justify-between">
        ...
        <div className=" grid gap-2 space-x-2">
          ...
          <button onClick={() => dispatch({ type: "ARCHIVE_NOTE", payload: id })}
            className="text-orange-600 cursor-pointer"
          >
            <RiInboxArchiveLine />
          </button>
          <button onClick={() => dispatch({ type: "PIN_NOTE", payload: id })}
            className="text-green-600 cursor-pointer"
          >
            {pinned ? <RiUnpinFill /> : <BsPinAngleFill />}
          </button>
          <button onClick={() => dispatch({ type: "DELETE_NOTE", payload: id })}
      ...
      {isTrashView && (
        <div className="flex justify-between mt-4 space-x-2">
          <button className="text-green-600 px-2 border rounded-lg focus:outline-2 cursor-pointer"
            onClick={() => dispatch({ type: "RESTORE_NOTE", payload: note.id })}
          >
            Restore
          </button>
          <button className="text-red-600 px-2 border rounded-lg focus:outline-2 cursor-pointer"
